...
while True:
    ...
    # формування Y
    y = [[round(3.5 + 6.6 * x[0][j] + 3.9 * x[1][j] + 1.8 * x[2][j] + 5.3 * x[0][j] * x[0][j] + 0.5 * x[1][j] * x[1][j] +
                4.3 * x[2][j] * x[2][j] + 6.0 * x[0][j] * x[1][j] + 0.8 * x[0][j] * x[2][j] + 9.4 * x[1][j] * x[2][j] +
    ...
    def table_fisher(prob, n, m, d):
        count_koef += len(importanceToPrint)
